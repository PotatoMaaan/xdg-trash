...
pub fn list_trashes(args: ListTrashesArgs) -> anyhow::Result<()> {
    ...
    let table = if !args.simple {
        Some(StreamingTable::draw_header([("Device ID", Some(9)), ("Type", Some(5)), ("Path", Some(15))]))
    } else {
        None
    };
    ...
    for trash in trashes {
        ...
        let location = trash.info_dir().parent().context("Info dir has no parent dir")?.to_string_lossy();

        if let Some(ref table) = table {
            table.draw_row([
                &dev, &trash_type, &location
            ]);
        } else {
